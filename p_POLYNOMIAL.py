...
dataset = pd.read_csv(r"D:\DatascienceNareshit\37. 08-01-25 Polynomial\1.POLYNOMIAL REGRESSION\emp_sal.csv")
...
#so the HR member of team runs the simple analysis on excel and observe that there is non -linear relationshipb/w the position level & salary
...
#so this employee is halfway b/w level 6 & level 7, so we can say that level is 6.5
...
#we encoded the position level associated with each position level 
...
#why we creating the 2 models because if we go backe to the equation & also compare the both results
...
poly_reg = PolynomialFeatures(4)   #we can change the degree here
...
#it will transfer untill you mention the power
#in one independent variable X  till 2,3,4 or 5 columns will be created based on the power
...
#first we will draw the visualtion of linear regression model and then we will build the visualize of polynomial regression model
